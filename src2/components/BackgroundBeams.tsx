...
export const BackgroundBeams = React.memo(({ className }: BackgroundBeamsProps) => {
  return (
    <div className={`pointer-events-none absolute inset-0 h-full w-full ${className || ''}`}>
      ...
      >
        {/* Static faint paths for depth (changed to very light grayish-blue to be visible on white bg instead of white-on-white) */}
        <g opacity="0.1">
          {pathData.map((d, i) => (
            <path key={`static-${i}`} d={d} stroke="#10296e" strokeWidth="0.5" />
          ))}
        </g>
        ...
        <defs>
          {pathData.map((_, i) => (
            <linearGradient
              ...
              y2="100%"
            >
              <stop offset="0%" stopColor="#0a3eff" stopOpacity="0" />
              <stop offset="20%" stopColor="#0a3eff" stopOpacity="1" />
